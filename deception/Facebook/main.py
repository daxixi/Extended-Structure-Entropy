...
for c in commus[1:]:
    cc=c.split(',')
    optional_attr[int(cc[0])]={'manu':int(cc[2])-1}
    cat=int(cc[2])-1
    if not cat in commuTonode:
        commuTonode[cat]=[int(cc[0])]
    else:
        commuTonode[cat].append(int(cc[0]))
nx.set_node_attributes(G,optional_attr)

#selected=np.random.choice(np.array(G.nodes()),1000)
#G=G.subgraph(selected.tolist())
...
# step is the communications add each step, default to 1
step=1
# 5000 is the cost, you can change it
for c in range(5000):
    print(c)
